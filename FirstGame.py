...
class Enemy:
	def __init__(self,pos,vel,accel,color):
		self.pos = pos
		self.vel = vel
		self.accel = accel
		self.color = color
...
class Game:
		...
		ball = Ball(initpos,initvel,initaccel)
		clock.tick(60)
		timeindex = 0
		timeindextwo = 0
		timeindexthree = 0
		...
		for index in range(0,numberenemies):
			...
			enemies.append(Enemy(Vector((width - enemyrad)*random.random(),menulength + (length-menulength-enemyrad)*random.random()),Vector(speed*random.random(),speed*random.random()),Vector(0,0),RandomColor(random.random())))
			index = index + 1
